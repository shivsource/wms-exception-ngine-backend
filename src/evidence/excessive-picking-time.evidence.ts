...
import { EvidenceCollector, ExcessivePickingTimeEvidence, PersistedException } from '../interfaces';
import { ExceptionType } from '../types/enums';
import { toOrderContext, toPickingTaskItemContext } from './shared';

export class ExcessivePickingTimeEvidenceCollector implements EvidenceCollector {
  ...
  private async build(exception: PersistedException): Promise<ExcessivePickingTimeEvidence | null> {
    ...
    const pickingMinutes = raw.pickingMinutes as number;
    const warehouseAverageMinutes = raw.warehouseAverageMinutes as number;
    ...
    return {
      task: { id: taskCode, taskCode, pickerId, pickingMinutes, warehouseAverageMinutes, ratio },
